...
DATE = '[0-3][0-9]/[01][0-9]/\d{2}'
TIME = '[0-2][0-9]:[0-5][0-9]:[0-5][0-9]'
TIMESTAMP = '%s %s' % (DATE, TIME)

LOG_PATTERN = '(?P<timestamp>%s) - (?P<message>[\w\s]+) - (?P<filename>[\w\.]+) : (?P<line>\d+) - (?P<level>\d+)' % (TIMESTAMP)
...
def main():
    ...
    parser.add_argument('--log', '-l', nargs='+', required=True, dest='logfiles',
                        help='path to MEN RTU log files')
    parser.add_argument('--name', '-n', required=True, dest='hostname',
    ...
    with open(args.csvfile, 'w') as csvfile:
        fieldnames = ['timestamp', 'message', 'filename', 'line', 'level', 'host']

...
def create_directory(directory):
    ...
    if not directory_exists(directory):
        try:
            ...
            logging.fatal("Failed to create a new directory %s", directory)
            return False
    return True
...
def search_and_replace(infile, search, replace):
    ...
    text = (
        open(infile, "rb")
        .read()
        .replace(bytes(search, "utf-8"), bytes(replace, "utf-8"))
    )
...
def prerequisites_exists(database_path, environment):
    ...
    archives_dir = os.path.join(database_path, "Archives", "DB_%s" % (environment))
    ...
    concentrator = os.path.join(
        database_path, "dac_DB_%s" % (environment), environment, "ScsDacCtrt.cfg"
    )
    if not file_exists(concentrator):
        return False

    xml_dir = os.path.join(database_path, "Database", "xml_DB_%s" % (environment))
    if not directory_exists(xml_dir):
        return False

    classlist = os.path.join(
        database_path, "Database", "classlist_%s.txt" % (environment)
    )
...
def create_export_directory(directory):
    """ """
    logging.info("Creating export directory for %s environment ...", directory)
    ...
    logging.info("Created the export directory %s successfully ...", directory)
    return True
...
def export_archives(database_path, environ):
    ...
    if environ == "CMS":
        ...
        export_item = os.path.join(export_path, "CMST_POWLIGHT", "CMST_POWLIGHT.cfg")
        ...
        export_item = os.path.join(export_path, "CMST_POWMXD", "CMST_POWMXD.cfg")
...
def patch_classlist(file_path):
    with open(file_path, "rb") as infile:
        content = infile.read()

    eqp_list = content.split("\n".encode("utf-8"))
    # C755B-1385
    # We moved LIFC at SKG from CMS to ECS database
    # As a consequence of this, PLC1 and PMSA need to be initialized
    # when ECS starts.
    eqp_list.append("PLC1".encode("utf-8"))
    eqp_list.append("PMSA".encode("utf-8"))

    uniq_eqp_list = []
    for eqp in sorted(eqp_list):
        if eqp != b"" and eqp not in uniq_eqp_list:
            uniq_eqp_list.append(eqp)

    with open(file_path, "wb") as outfile:
        for eqp in uniq_eqp_list:
            item = "{}\n".format(eqp.decode("utf-8"))
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        dest="fix_line_ending",
        help="fix Windows line-ending",
    )
    ...
    for environ in args.environment:
        ...
        logging.info("Prepping database files for %s environment complete ...", environ)

    if args.db_version != None:
        for environ in args.environment:
            if directory_exists(environ):
                outfile = "NELDB_%s_C755B_%s.tar.gz" % (environ, args.db_version)
                logging.info(
                ...
                logging.warning("%s does not exist. Skipping ...", environ)

    if args.fix_line_ending:
        for environ in args.environment:
            # dbmuserconst_class_*.cfg files are at args.database_path
            class_cfg = os.path.join(
                args.database_path, "Database", "dbmuserconst_class_%s.cfg" % (environ)
            )
            classlist = os.path.join(
                args.database_path, "Database", "classlist_%s.txt" % (environ)
            )
            if file_exists(class_cfg):
                logging.info(
                    "Converting Windows line ending to UNIX line ending in %s...",
                    class_cfg,
                )
                fix_file_line_ending(class_cfg)

            if file_exists(classlist):
                if classlist == "../Database/classlist_ECS.txt":
                    logging.info("Patching %s...", classlist)
                    patch_classlist(classlist)
